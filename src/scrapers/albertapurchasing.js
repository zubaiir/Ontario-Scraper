...
const BASE_LIST_URL = 'https://purchasing.alberta.ca/';
const PORTAL_SOURCE = 'Alberta Purchasing Connection';

async function scrapeAlberta({ page, maxItems = 60 }) {
  console.log('🔍 Starting Alberta Purchasing Connection scrape...');
  ...
  await waitForAnySelector(page, [
    'table tbody tr a[href*="Opportunity"]',
    'table tbody tr',
  ]);
  ...
  const listItems = await page.$$eval(
    ...
    (rows, { portalSource }) => {
      ...
      for (const row of rows) {
        const linkEl =
          row.querySelector('a[href*="Opportunity"]') ||
          row.querySelector('a');
        if (!linkEl) continue;
        ...
        const agency =
          row.querySelector(
            'td[data-title="Organization"], td[data-title="Ministry"]'
          )?.textContent.trim() || '';
        ...
        const created_at =
          row.querySelector(
            'td[data-title="Issue Date"], td[data-title="Posted"]'
          )?.textContent.trim() || '';
        ...
        const project_reference =
          row.querySelector(
            'td[data-title="Reference"], td[data-title="Solicitation"]'
          )?.textContent.trim() || '';
  ...
  if (!listItems.length) {
    console.log('No rows detected for Alberta Purchasing Connection.');
    return results;
  }
    ...
    try {
      ...
      const detailData = await page.evaluate(() => {
        ...
        const text = Array.from(
          document.querySelectorAll('.opportunity-details, .content, main')
        )
          .map((el) => el.innerText || '')
          .join('\n');
        ...
        const detailed_description =
          document.querySelector('.opportunity-details, .content, main')
            ?.innerText.trim().slice(0, 4000) || '';

        return {
          project_reference_detail:
            getByLabel(['Reference', 'Solicitation']) || '',
  ...
  console.log(
    `\n🏁 Alberta Purchasing Connection scrape complete with ${results.length} records.`
  );
  return results;
}
