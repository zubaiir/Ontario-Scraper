...
const BASE_URL = 'https://www.bidcentral.ca/';
const PORTAL_SOURCE = 'BidCentral';

async function scrapeBidCentral({ page, maxItems = 40 }) {
  console.log('🔍 BidCentral start');
  const results = [];
  ...
  await waitForAny(page, [
    '.opportunity', '.project', 'a[href*="opportunities"]'
  ], 15000).catch(() => {});
  ...
  for (const item of sliced) {
    try {
      ...
      const fp = generateFingerprint(`${merged.title}${merged.portal_url}BidCentral`);
      results.push({ id: fp, ...merged, hash_fingerprint: fp });
    } catch (e) {
      console.warn(`⚠️ BidCentral detail: ${e.message}`);
    }
  }

  console.log(`🏁 BidCentral done with ${results.length} records`);
  return results;
}
...
module.exports = { scrapeBidCentral };
