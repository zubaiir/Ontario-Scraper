...
const BASE_URL = 'https://amp.quebec/';
const PORTAL_SOURCE = 'AMP Québec';

async function scrapeAmpQuebec({ page, maxItems = 40 }) {
  console.log('🔍 AMP Québec start');
  const results = [];
  ...
  const listItems = await page.$$eval(
    'a[href*="appel-offres"], a[href*="tender"]',
    (links, { portalSource }) => {
      const items = [];
      const seen = new Set();
  ...
  for (const item of sliced) {
    try {
      ...
      const fp = generateFingerprint(`${merged.title}${merged.portal_url}AmpQuebec`);
      results.push({ id: fp, ...merged, hash_fingerprint: fp });
    } catch (e) {
      console.warn(`⚠️ AMP Québec detail: ${e.message}`);
    }
  }

  console.log(`🏁 AMP Québec done with ${results.length} records`);
  return results;
}
...
module.exports = { scrapeAmpQuebec };
